...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f3f4f6;">
    ...
    def send_action_notification(self, call_data: Dict, recipient_email: str) -> Dict:
        ...
        The email content is dynamically generated by AI in the customer's detected language.
        """
        priority_level = call_data.get("final_decision", {}).get("priority_level", "MEDIUM")
        
        # Get language information from call data
        language = call_data.get("language", "en")
        language_name = call_data.get("language_name", "English")
        
        # Generate subject in customer's language if possible
        subject_templates = {
            "ta": f"🎯 நடவடிக்கை தேவை: {priority_level.upper()} முன்னுரிமை அழைப்பு",
            "ml": f"🎯 നടപടി ആവശ്യമാണ്: {priority_level.upper()} മുൻഗണന കോൾ",
            "hi": f"🎯 कार्रवाई आवश्यक: {priority_level.upper()} प्राथमिकता कॉल",
            "te": f"🎯 చర్య అవసరం: {priority_level.upper()} ప్రాధాన్యత కాల్",
            "en": f"🎯 Action Required: {priority_level.upper()} Priority Call"
        }
        subject = subject_templates.get(language, subject_templates["en"])
        
        # Generate email content using AI in detected language
        if self.llm_service:
            try:
                html_content = self.llm_service.generate_action_email(
                    call_data,
                    language=language,
